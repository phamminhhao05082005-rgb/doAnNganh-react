...
const StudentEducations = () => {
    ...
    const openEdit = (e) => {
        ...
        setForm({
            ...
            major: e.major || "",
            degree: e.degree || "",
            gpa: e.gpa !== null && e.gpa !== undefined ? String(e.gpa) : "",
    ...
    const save = async () => {
        const { school_name, major, degree, gpa, start_date, end_date } = form;

        if (!school_name || !major || !degree || gpa === "" || gpa === null || !start_date || !end_date) {
            alert("Vui lòng không để trống bất kỳ trường nào.");
            return;
        }

        if (isNaN(Number(gpa))) {
            alert("GPA chỉ được điền số.");
            return;
        }
    ...
    return (
        <Card className="mt-4 shadow">
            <Card.Header className="d-flex justify-content-between">
                ...
                <Button onClick={openCreate}>Thêm học vấn</Button>
            </Card.Header>
            <Card.Body>
                    ...
                    <tbody>
                        {educations.map(e => (
                            <tr key={e.id}>
                                ...
                                <td>{e.major}</td>
                                <td>{e.degree || "-"}</td>
                                <td>{e.gpa !== null && e.gpa !== undefined ? e.gpa : "-"}</td>
            ...
            <Modal show={show} onHide={() => setShow(false)}>
                <Modal.Header closeButton>
                    <Modal.Title>
                        {editingId ? "Cập nhật học vấn" : "Thêm học vấn"}
                    </Modal.Title>
                </Modal.Header>
                <Modal.Body>
