...
const StudentExperiences = () => {
    ...
    const loadExperiences = async () => {
        try {
            const res = await authApis().get(endpoints.experiences);
    ...
    const openEdit = (e) => {
        ...
        setForm({
            ...
            position: e.position || "",
            start_date: e.start_date ? dayjs(e.start_date).format("YYYY-MM-DD") : "",
            end_date: e.end_date ? dayjs(e.end_date).format("YYYY-MM-DD") : "",
    ...
    const save = async () => {
        const { company_name, position, start_date, description } = form;

        // Validate không được để trống (bỏ qua end_date vì có thể là "Hiện tại")
        if (!company_name || !position || !start_date || !description) {
            alert("Vui lòng không để trống Công ty, Vị trí, Ngày bắt đầu và Mô tả.");
            return;
        }
    ...
    return (
        <Card className="mt-4 shadow">
            <Card.Header className="d-flex justify-content-between">
                ...
                <Button onClick={openCreate}>Thêm kinh nghiệm</Button>
            </Card.Header>
                    ...
                    <tbody>
                        {experiences.map(e => (
                            <tr key={e.id}>
                                ...
                                <td>{e.position}</td>
                                <td>{dayjs(e.start_date).format("DD/MM/YYYY")}</td>
                                <td>
                                    {e.end_date
            ...
            <Modal show={show} onHide={() => setShow(false)} size="lg">
                ...
                <Modal.Body>
                    ...
                    <Form.Group className="mb-3">
                        <Form.Label>Ngày kết thúc (Để trống nếu là hiện tại)</Form.Label>
                        <Form.Control
                            type="date"
                            value={form.end_date}
